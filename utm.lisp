...
(defun lat-lon-to-utm (lat lon &key (ellipsoid "WGS84") (zone nil))
  ...
  (declare (optimize (speed 3) (space 0) (safety 3) (debug 0)))
  ...
  (let* ((lat-rad (deg2rad lat))
         ...
         (nu (realpart (/ a (expt (- 1.0 (* e-squared (expt (sin lat-rad) 2.0d0))) 0.5d0))))
         (p (- lon-rad long0))
...
(defun deg-min-sec-to-decimal (degree minute second)
  ...
  (declare (optimize (speed 3))
           (type double-float degree minute second))
  (+ degree (/ minute (/ second 60.0d0) 60.0d0)))
